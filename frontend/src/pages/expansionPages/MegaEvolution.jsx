...
const MegaEvolutionPage = () => {
  ...
  // Fetch Mega Evolution cards from backend
  useEffect(() => {
    const fetchCards = async () => {
      setLoading(true);
      try {
        const res = await fetch(`/api/cards/mega-evolution?page=${page}&pageSize=${pageSize}`);
        ...
        console.error("Failed to fetch Mega Evolution cards:", err);
      } finally {
        setLoading(false);
      }
    };
    ...
    <div className="relative flex flex-col min-h-screen text-white bg-neutral-900">
      <div className="relative z-10">
        ...
        <div className="w-full p-4 shadow-md bg-linear-to-r from-slate-950 via-red-950/60 to-slate-950">
          <h2 className="text-white text-xl font-bold">Mega Evolution</h2>
        </div>
