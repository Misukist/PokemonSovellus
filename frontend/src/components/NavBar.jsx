    ...
    <div
      className={`w-full py-4 px-2 top-0 z-50 shadow-xl/20 transition-all duration-300 ${
        isCards || isMega || isPhantasmal || isScarlet || isMyCollection
          ? "sticky bg-black shadow-lg"
          : "fixed bg-black/60 backdrop-blur-3xl"
      }`}
    >
      ...
      {/* Mobile menu */}
      {isMobileMenuOpen && (
        <div className="md:hidden mt-2 bg-black/90 backdrop-blur-xl rounded-lg p-4 flex flex-col space-y-2">
          <DropdownExpansion />
          <Link to="/cards" className="text-md font-bold text-white sm:hover:border-indigo-400 hover:text-red-800 sm:hover:bg-transparent rounded-lg p-1">
            Cards
          </Link>
